...
async def system_init(
    ...
    if loader:
        ...
        for manifest in loader.get_loaded_modules():
            ...
            if is_admin or (has_perm and manifest.enabled):
                modules.append({
                    "id": manifest.id,
                    "name": manifest.name,
                ...
                # 收集需要加载的模块资源
                if manifest.enabled and hasattr(manifest, 'assets'):
            ...
            # 菜单：仅启用且有权限
            if manifest.enabled and manifest.menu:
                if not has_perm:
                    continue
                menus.append({
                    "module": manifest.id,
...
async def cleanup_old_jwt_secret(
    ...
    注意：清理后旧Token将无法验证，请确保所有旧Token已过期
    """
    # 检查是否为系统管理员
    if current_user.role != "admin":
        raise HTTPException(status_code=403, detail="仅系统管理员可执行此操作")
