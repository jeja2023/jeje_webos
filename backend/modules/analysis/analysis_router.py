...
async def create_smart_report(
    ...
    try:
        ...
        return error(format_error_message(e, "创建报告模板失败"))
...
async def generate_smart_report(
    ...
    try:
        ...
        elapsed = time.time() - start_time
        logger.info(f"报告生成成功: report_id={report_id}, 耗时: {elapsed:.2f}秒")
        ...
        # 参数错误
        elapsed = time.time() - start_time
        logger.warning(f"报告生成参数错误: report_id={report_id}, 耗时: {elapsed:.2f}秒, 错误: {e}")
        return error(f"参数错误: {str(e)}")
    except FileNotFoundError as e:
        # 文件未找到
        elapsed = time.time() - start_time
        logger.error(f"报告生成文件错误: report_id={report_id}, 耗时: {elapsed:.2f}秒, 错误: {e}")
        return error(f"文件错误: {str(e)}")
    except Exception as e:
        # 其他异常
        elapsed = time.time() - start_time
        logger.error(f"报告生成失败: report_id={report_id}, 耗时: {elapsed:.2f}秒, 错误: {e}", exc_info=True)
        # 返回更友好的错误信息
        error_msg = str(e)
