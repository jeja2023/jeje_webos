...
class IMConversationMember(Base):
    ...
    joined_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=lambda: datetime.now(timezone.utc), comment="加入时间")
...
class IMMessage(Base):
    ...
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=lambda: datetime.now(timezone.utc), index=True, comment="发送时间")
...
class IMMessageRead(Base):
    ...
    user_id: Mapped[int] = mapped_column(Integer, ForeignKey(User.id), index=True, comment="用户ID")
    conversation_id: Mapped[int] = mapped_column(Integer, ForeignKey("im_conversations.id", ondelete="CASCADE"), index=True, comment="会话ID")
    read_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=lambda: datetime.now(timezone.utc), comment="阅读时间")
