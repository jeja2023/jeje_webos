...
CHANGELOG: List[ChangelogEntry] = [
    ChangelogEntry(
        ...
        changes={
            "feature": [
                "新增注册审核开关，支持管理员审核与自动通过注册模式",
                "新增新用户默认存储配额设置"
            ],
            "improvement": [
                "新用户默认进入普通用户组",
                "去除访客作为内置默认用户组",
                "同步优化用户、角色、个人资料、帮助与系统设置页面展示"
            ],
            "security": [
                "注册接口直接读取最新系统设置，避免缓存延迟影响审核策略",
                "收紧注册限流并增加 IP、用户名、手机号维度防刷检查"
            ],
            "fix": [
                "修复关闭注册审核后仍可能等待管理员审核的问题",
                "修复系统设置缓存刷新不及时的问题",
                "修复不同入口创建用户时默认角色与存储配额不一致的问题"
            ]
        }
    ),
    ChangelogEntry(
