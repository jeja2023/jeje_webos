...
def register_frontend_route(module_id: str, module_name: str, model_class: str, frontend_dir: Path):
    ...
    # 检查是否已经注册过该模块的路由
    if f"'/{module_id}':" in content or f"'/{module_id}/list':" in content:
        print(f"  [跳过] 路由已存在: /{module_id}")
        return False
    
    # 生成简化的路由代码（只需要主路由和列表页）
    route_code = f'''
            ...
            '/{module_id}/list': {{
                ...
                handler: () => {{
                    ...
                    if (typeof {model_class}Page !== 'undefined') {{
                        this.currentPage = new {model_class}Page(this.content);
                        this.currentPage.mount();
                    }} else {{
                        this.content.innerHTML = '<div class="error-message">{model_class}Page 组件未定义。</div>';
                    }}
                    this.setWindowTitle('{module_name}');
                }}
            }},'''
    
    # 策略1：已移除 (原策略依赖正则匹配 /apps 路由，容易出错导致插入到对象外部)
    pass
    
    # 策略2：查找 registerRoutes 方法中的 registerAll 闭合位置
    
    # 策略2：查找 registerRoutes 方法中的 registerAll 闭合位置
    # 需要找到 registerRoutes() 方法内部的 }); 而不是其他方法的
    
    # 找到 registerRoutes 方法的开始
    register_routes_match = re.search(r'registerRoutes\s*\(\s*\)\s*\{', content)
    if not register_routes_match:
        print(f"  [警告] 无法找到 registerRoutes 方法")
        return False
    
    register_routes_start = register_routes_match.end()
    
    # 在 registerRoutes 方法范围内找到 Router.registerAll 的闭合 });
    # 从 registerRoutes 开始向后查找
    search_content = content[register_routes_start:]
    
    # 查找 }); 模式（registerAll 的闭合）
    # 使用更宽松的匹配 \s* 以适应不同的格式化
    registerall_close = re.search(r'\n\s*\}\);', search_content)
    
    if registerall_close:
        # 计算实际插入位置 (在 }); 之前)
        insert_pos = register_routes_start + registerall_close.start()
        
        # 智能检查是否需要补逗号 (忽略注释和空白)
        needs_comma = False
        
        # 获取插入点之前的内容并按行分割
        pre_content = content[:insert_pos]
        ...
        # 如果最后一个有效字符不是逗号、左大括号或左中括号，则需要补逗号
        if last_char and last_char not in [',', '{', '[']:
            needs_comma = True
        
        if needs_comma:
            # 在插入代码前加逗号 (前置换行防止被注释吞没)
            new_content = pre_content + '\n,' + route_code + content[insert_pos:]
        else:
            new_content = pre_content + route_code + content[insert_pos:]
        
        app_js_path.write_text(new_content, encoding='utf-8')
        print(f"  [完成] 路由已注册: /{module_id}/list")
        return True
    
    print(f"  [警告] 无法找到路由插入位置，请手动添加路由到 app.js 的 registerRoutes 方法中")
    return False
