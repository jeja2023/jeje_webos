...
- **JWT Token 体积优化（502 修复）**:
  ...
  - 新增 `compress_permissions()` 函数，在生成 JWT 时自动压缩权限列表：移除被通配符（如 `notes.*`）已覆盖的细粒度权限（如 `notes.read`），显著减小 Token 体积。
  - 前端 UI 仍返回完整权限列表用于页面渲染，后端每次请求时通过 `_sync_user_permissions` 从数据库实时加载完整权限，确保权限判断不受影响。
