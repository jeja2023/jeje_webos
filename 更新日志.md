...
### 🚀 功能增强 (Enhancements)
- **全局搜索 (Spotlight) 深度增强**:
  - **搜索范围扩展**: 快捷搜索现在支持 **用户 (Users)**、**笔记 (Notes)**、**知识库 (Knowledge)**、**相册 (Album)** 和**数据透镜 (DataLens)** 的动态搜索。
  - **并发搜索优化**: 引入 `Promise.allSettled` 并发机制，同步发起多个模块的搜索请求，显著提升搜索响应速度，不再受限于单一接口延迟。
  - **静态索引更新**: 重新整理并增加了 10+ 个常用应用与设置的快速跳转入口。
  - **视觉提升**: 搜索结果支持显示用户头像，优化了图标渲染逻辑与分组显示效果。
