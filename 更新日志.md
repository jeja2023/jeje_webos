## [v2.5.35] - 2026-05-26 (部署启动热修复)

### 修复

- **psutil 导入修复**: 移除项目根目录下遮蔽第三方包的 `backend/psutil.py` 兼容文件，修复容器启动阶段 `psutil._common` 循环导入报错的问题。
- **部署稳定性**: 新增 `utils.psutil_compat` 显式兼容入口；Docker 环境优先使用官方 `psutil>=5.9.0`，本地未安装时自动降级为轻量监控实现。

### 验证

- 通过 `python -B -c "from utils.psutil_compat import psutil; ..."` 验证监控兼容入口可正常导入。
- 通过 `python -B -c "import main"` 验证应用入口可正常导入。
- 通过 `backend/tests/test_health_checker.py` 与 `backend/tests/test_config.py` 测试。

---

## [v2.5.34] - 2026-05-26 (注册审核、用户组与默认配额安全优化)

### 新增

- **注册审核开关**: 在系统设置中新增“注册需要审核”开关，支持管理员审核注册与自动通过注册两种模式。
- **默认存储配额**: 新增新用户默认存储配额设置，用户注册、后台创建用户、批量导入用户时都会自动写入默认配额。

### 优化

- **默认用户组统一**: 新注册用户在管理员审核模式和自动通过模式下均默认进入“普通用户”组。
- **用户组初始化优化**: 去除“访客”作为内置默认用户组，仅保留“系统管理员、管理员、普通用户”三类内置角色。
- **历史数据兼容**: 启动时自动将历史访客用户迁移到普通用户组，并清理遗留访客系统组，避免权限模型继续分叉。
- **前端管理体验**: 同步更新用户、角色、个人资料、帮助与系统设置页面的用户组展示和注册设置交互。

### 安全

- **注册设置实时生效**: 修复关闭注册审核后仍可能读取旧缓存，导致用户注册继续等待审核的问题。
- **注册接口鲜读配置**: 注册流程直接从数据库读取最新注册策略，避免系统设置缓存延迟影响安全决策。
- **注册限流收紧**: 注册接口限流调整为 3 次/分钟，超限后阻断 30 分钟，降低恶意批量注册风险。
- **多维度防刷**: 增加 IP、用户名、手机号维度的注册频次检查；启用 Redis 缓存时可进一步拦截短时间批量注册尝试。

### 修复

- **系统设置缓存刷新**: 修复系统设置保存后缓存未及时刷新/失效的问题，确保注册审核、默认配额等配置保存后立即生效。
- **默认角色一致性**: 修复不同入口创建用户时默认角色、用户组和存储配额不一致的问题。

### 验证

- 通过 Python AST 语法检查。
- 通过前端页面脚本 `node --check` 检查。
- 通过 `backend/tests/test_bootstrap.py` 与 `backend/tests/test_health_checker.py` 测试。
- 通过 `git diff --check` 空白检查（仅保留既有换行格式提示）。

---

## [v2.5.33] - 2026-02-12 (安全加固与性能稳定性优化)

### 🛡️ 安全加固 (Security Hardening)

- **SQL 注入防护 (DataLens)**:
  - 在 `backend/modules/datalens/datalens_services.py` 中增加了严格的列名安全校验 (`is_safe_column_name`)。
  - 修复了筛选 (`_build_filter_clause`) 和排序 (`_build_sort_clause`) 逻辑中直接拼接字段名的潜在风险，确保仅允许合法的数据库列名通过。

- **用户查询健壮性**:
  - `backend/core/security.py`：将 `scalar_one_or_none()` 替换为 `scalars().first()`，在获取用户信息时提高容错性，防止因意外的多条记录导致 500 错误。

- **CSRF 逻辑清晰化**:
  - `backend/core/csrf.py`：移除了未使用的 `used` 字段逻辑，明确了 CSRF Token 仅基于过期时间（3600秒）进行验证的设计，消除了代码歧义。

### 🚀 性能与稳定性 (Performance & Stability)

- **并发安全修复**:
  - 修复了 `backend/modules/notes/notes_services.py` 中使用 `asyncio.gather()` 并发执行共享 Session 的数据库查询导致的潜在不稳定性。
  - 将统计接口 (`get_stats`) 的并发查询回退为串行执行，确保 SQLAlchemy 会话的线程安全。

- **启动性能优化**:
  - `backend/routers/boot.py`：优化了系统初始化时的统计查询，将多个独立的 `SELECT COUNT(*)` 查询合并为单个 SQL 语句执行。
  - 减少了数据库往返次数 (Round-trip)，提升了首屏接口的响应速度。

- **前端标准统一**:
  - `frontend/js/pages/pdf/pdf.js`：统一了 API 响应状态码检查逻辑，将不一致的 `res.code === 0` 修正为标准 `res.code === 200`，修复了部分操作误报失败的问题。

### 🐛 Bug 修复 (Bug Fixes)

- **根路由资源加载修复**:
  - 修复了访问网站根路径 (`/`) 时，`loader.js` 错误尝试加载不存在的 `index.css` 和 `index.js` 导致 404 错误的问题。
  - 在 `loader.js` 的 `moduleResourceMapOverrides` 中增加了根路径的空资源映射，明确告知加载器根路径无需加载额外资源。

---

## [v2.5.32] - 2026-02-12 (架构优化：统一异常体系、前端日志与资源加载约定)

### 🏗️ 架构与规范优化 (Architecture & Standards)

- **后端统一异常体系 (AppException)**：
  - 全站路由与模块由 `HTTPException` 迁移为 `core.errors` 下的 `AppException` 体系，统一返回 `{ code, message, data }`，与前端 `api.js` 的 `data.code !== 200` 解析一致。
  - 涉及：`core/security.py`、`routers/`（auth、user、storage、boot、import_export、audit、roles、announcement、backup、notification、market、websocket）、`modules/`（feedback、blog、notes、markdown、pdf、video、transfer、knowledge、im、filemanager、album、datalens、vault、exam、course）、`main.py`、`utils/auth_helpers.py`。
  - 404 → `NotFoundException("资源名")`，403 → `PermissionException(...)`，401 → `AuthException(ErrorCode.xxx, ...)`，400/413/500 → `BusinessException(ErrorCode.xxx, ...)`。
  - 例外：`rate_limit.py` 保留 `HTTPException(429, headers={"Retry-After": ...})` 以支持限流响应头。

- **前端 API 封装优化 (api.js)**：
  - 抽取 Token 刷新逻辑为内部方法 `_tryRefreshToken()`，供 `request()` 与 `upload()` 复用，消除重复代码并保证 401 处理与重试行为一致。

- **前端统一日志出口 (Config.log / Config.warn / Config.error)**：
  - 业务与组件中 `console.log` / `console.warn` / `console.error` 已统一替换为 `Config.log`、`Config.warn`、`Config.error`（带 `typeof Config !== 'undefined'` 防护），便于通过 `Config.debug` 控制输出及后续接入监控。
  - `loader.js` 中 CSS/JS 加载失败、ECharts/Leaflet 加载提示也改为使用 Config 或 `this.log`。

- **前端资源加载约定 (loader.js)**：
  - **约定**：路由 `/path` 默认对应 `/static/js/pages/{path}.js` 与 `/static/css/pages/{path}.css`，新增符合该约定的页面无需再改 loader 配置。
  - **覆盖表**：仅对不符合约定的路由（如 `/theme`→theme_editor、`/apps`→market、`/pdf`/`/markdown`/`/analysis`/`/lens` 等多文件或子目录）在 `moduleResourceMapOverrides` 中配置。
  - **动态模块**：`registerDynamicModules()` 写入 `moduleResourceMapDynamic`，`getResourcesForPath(basePath)` 按「覆盖表 → 动态表 → 约定生成」顺序解析资源。

### 🐛 Bug 修复 (Bug Fixes)

- **页面组件加载修复**: 
  - 修复了“系统监控”、“数据备份”和“权限管理”页面由于缺少显式资源映射导致的“页面组件未定义”错误。
  - 在 `loader.js` 中由于子路径未在覆盖表中列出，导致无法正确加载对应的微前端模块。已补全 `/system/monitor`, `/system/backup`, `/system/roles` 的资源映射。
- **资源加载逻辑优化**: 
  - 改进了 `ResourceLoader.loadModuleByPath` 的匹配逻辑，优先匹配完整路径覆盖项，再回退到基路径映射，确保了子路由页面资源加载的精确性。
- **安全与体验优化**: 
  - 为用户管理模块 (`users.js`) 的“添加用户”和“重置密码”表单中的密码输入框增加了 `autocomplete="new-password"` 属性，消除了浏览器关于“缺少自动完成属性”的安全与可用性警告。

### 📋 文档与规范 (Documentation)

- **开发规范**：新增「3.5.2 业务异常统一使用 AppException 体系」「5.5.4 前端日志规范」「5.5.5 资源加载器约定与覆盖表」，便于后续开发与协作统一。

---

## [v2.5.31] - 2026-02-12 (PDF模块全能增强：预览修复与图片/Office文档直接查看)

### 🚀 功能增强 (Enhancements)

- **PDF 工具箱全能预览**:
  - **图片直接预览**: 在 PDF 工具箱中点击图片文件（`.jpg`, `.png`, `.bmp` 等），现在可以直接通过精美的模态框查看大图，不再提示转换为 PDF。
  - **Office 文档预览**: 集成 `OfficeViewer` 组件，支持在 PDF 工具箱中直接点击预览 Word (`.docx`) 和 Excel (`.xlsx`) 文件，无需下载或转换。
  - **API 支持**: 后端 PDF 模块新增 `/preview` 接口，支持安全地获取模块内的图片和文档流。

- **PDF 阅读器体验升级**:
  - **悬浮工具栏**: 重构 PDF 阅读器 UI，底部工具栏改为悬浮设计，采用磨砂玻璃效果，视觉更加现代化且不遮挡内容。
  - **交互优化**: 移除了底部工具栏中重复的“关闭”按钮，统一使用右上角关闭操作，界面更加简洁。

### 🐛 Bug 修复 (Bug Fixes)

- **PDF 翻页空白修复**:
  - 修复了 PDF 阅读器在翻页时显示空白的问题。
  - **原因**: DOM 替换逻辑导致图片元素的 `load` 事件监听器丢失。
  - **修复**: 优化 `_refreshPage` 逻辑，仅更新图片 `src` 和文本内容，保留 DOM 元素和事件绑定，确保翻页平滑且内容正常加载。

- **样式加载修复**:
  - 修复了 PDF 阅读器样式文件 (`pdf_viewer.css`) 未在文件管理器模块中加载的问题，导致阅读器样式错乱。
  - 将 `pdf_viewer.css` 和 `office_viewer.css` 正确添加到 `loader.js` 的模块依赖中。

- **日志降噪**:
  - 移除了后端数据分析模块中查询成功时的 redundant INFO 日志，保持控制台清爽。

- **错误处理规范化**:
  - 优化了 PDF 模块后端 (`pdf_router.py`) 的异常处理，使用标准的 `BusinessException`、`PermissionException` 和 `NotFoundException` 替代通用的 `HTTPException`，符合项目开发规范。

---

## [v2.5.30] - 2026-02-12 (性能优化：首屏加载提速与数据库初始化优化)

### 🚀 性能优化 (Performance Optimization)

- **首屏加载提速 (Frontend Load Speed)**:
  - **资源懒加载**: 重构 `index.html`，移除所有业务模块（系统管理、文件管理、媒体工具、通讯AI、数据分析等 20+ 个模块）的硬编码脚本引用。
  - **按需加载**: 全面启用 `ResourceLoader` 机制，仅在用户访问特定路由时动态加载对应的 JS 资源，显著减少首屏 HTTP 请求数和下载体积。
  - **核心保留**: 保留了核心基础库（Router, Store, API）和关键页面（登录、桌面）的预加载，保障应用启动稳定性。

- **后端启动优化 (Backend Startup)**:
  - **数据库初始化加速**: 优化 `backend/core/database.py` 中的 `init_db` 逻辑。
  - **幂等建表**: 将 `CreateTable` 操作改为 `IF NOT EXISTS` 模式；`CreateIndex` 保持异常兼容模式以适配标准 MySQL 语法，移除冗余的重试逻辑。
  - **日志降噪**: 消除了服务启动时因表已存在而产生的大量 `OperationalError` 异常和日志噪音，提升启动效率。

### 🐛 Bug 修复 (Bug Fixes)

- **前端懒加载组件错误**:
  - 修复了启用资源懒加载后，因 ES6 类未挂载到全局作用域导致的 `ReferenceError` 问题。
  - 编写了自动化脚本，将所有业务页面组件（Knowledge, Market, Note 等 30+ 个模块）显式导出到 `window` 对象，确保 `ResourceLoader` 动态加载后可被路由正确实例化。
  - 标准化了所有自动生成的代码注释为中文。

- **MySQL 兼容性修复**:
  - 修复了数据库初始化脚本中 `CREATE INDEX IF NOT EXISTS` 语法导致的 MySQL 语法错误（MySQL 不支持索引创建时的 `IF NOT EXISTS`）。
  - 调整为标准的异常捕获模式，确保在各类数据库环境下的兼容性。

---

## [v2.5.29] - 2026-02-11 (消息通知增强：实时审核提醒与徽标联动)

### ✨ 新增功能 (New Features)

- **实时消息通知 (Real-time Audit Notification)**:
  - **新用户注册提醒**: 新增 `backend/core/event_handlers.py` 模块，监听 `USER_REGISTER` 事件。当新用户注册时，系统将通过 WebSocket 实时推送消息给所有在线管理员。
  - **待审核徽标联动**: 在接收到新用户注册通知时，前端顶栏（TopBar）会自动刷新待审核徽标数量，无需手动刷新页面。

### ⚡ 体验优化 (UX Improvements)

- **管理员密码重置流程优化**:
  - 简化重置密码弹窗，移除冗余的“确认密码”输入框。
  - 新增“随机密码生成”功能，默认生成强密码并自动选中，便于管理员快速复制并发送给用户。

### 🐛 Bug 修复 (Bug Fixes)

- **审核徽标同步 (Notification Badge Sync)**:
  - 修复管理员在 `PendingUsersPage` 或 `UserListPage` 执行审核操作（通过/拒绝）后，顶栏待审核徽标不立即更新的问题。
  - **前端逻重构**: 
    - `users.js`: 在审核操作成功后显式调用 `UserApi.getPendingUsers()` 获取最新待审核数，并同步至全局 Store。
    - `topbar.js`: 增加对 `Store.pendingCount` 的订阅监听，确保任何来源的数据变更都能即时反映在 UI 上。
    - `websocket.js`: 增强通知处理逻辑，识别 `category: user_pending` 类型的通知并自动刷新计数。
  - **后端修复**: 修复 `lifespan.py` 中因之前的错误操作引入的重复异常处理块。

---

## [v2.5.28] - 2026-02-11 (合规性增强：ICP/公安备案挂载支持)

### ✨ 新增功能 (New Features)

- **系统合规性 (Compliance)**:
  - **ICP 备案信息**: 新增 ICP 备案号及查询链接配置项。配置后可在登录页底部展示，并自动跳转至工信部备案查询系统。
  - **公安备案信息**: 新增公安机关互联网站备案号及查询链接配置项。支持展示标准公安备案图标，并提供外部跳转链接。
  - **动态配置管理**: 管理员可在“系统设置”页面实时修改备案信息，无需重启服务或手动修改代码。

- **认证与安全加固 (Authentication & Security)**:
  - **Auth 路由重构**: 深度重构 `auth.py`，移除冗余代码与调试性日志，提升代码纯净度与可维护性。
  - **权限逻辑统一**: 提取 `resolve_permissions` 共享函数，实现登录、个人信息获取及令牌刷新三处权限汇总逻辑的完全对齐（直接权限 ∪ 角色权限）。
  - **审计日志增强**: 为注册、登录、登出、改密等核心认证环节补齐系统级审计日志，确保安全事件可追溯。
  - **IP 安全溯源**: 全站认证接口统一采用 `get_client_ip` 工具进行 IP 解析与清洗，支持代理转发环境并防止 IP 伪造。
  - **暴力破解防护**: 强化账户级锁定机制与限流策略，结合“用户名或密码错误”的模糊化提示，有效防御用户名枚举与自动化暴力攻击。
  - **登出流程优化**: 完善 HttpOnly Cookie 模式下的令牌清理逻辑，确保会话注销的彻底性。

### 🎨 界面优化 (UI Improvements)

- **登录页页脚**: 重写登录页页脚布局，支持响应式显示备案信息，保持视觉简约与专业感。

### 🐛 Bug 修复 (Bug Fixes)

- **密码哈希校验修复**:
  - 修复批量导入用户时因密码哈希为空或格式无效导致 `bcrypt` 验证崩溃（502 Bad Gateway）的问题。
  - 在 `core/security.py` 中增加 `verify_password` 的健壮性检查，确保仅对有效哈希（以 `$` 开头）执行验证，无效则安全返回 False。
- **旧版密码兼容性修复**:
  - 修复了新版哈希验证逻辑在返回 False（不匹配）时未能回退到旧版验证逻辑，导致老用户（无预哈希密码）无法登录的问题。
  - 重构 `verify_password` 流程，确保新版验证失败后自动尝试旧版验证，保障全量用户登录正常。
- **JWT Token 体积优化（502 修复）**:
  - 修复非管理员用户登录时因 JWT Token 中包含大量细粒度权限（100+项），导致 Set-Cookie 响应头超过 Nginx/OpenResty 代理缓冲区限制（默认 4KB），触发 502 Bad Gateway 的问题。
  - **架构优化**：JWT Token 中不再存储权限列表，仅保留核心身份信息（user_id、username、role），Token 大小恒定（~200字节），不受模块数量影响。
  - 后端每次请求时通过 `_sync_user_permissions` 从数据库实时加载权限（带 120 秒 TTL 缓存），确保权限变更即时生效。
  - 前端仍通过登录/刷新响应体获取完整权限列表用于 UI 渲染，功能不受影响。
  - 新增 `compress_permissions()` 工具函数，移除被通配符覆盖的冗余权限，供其他场景使用。

---

## [v2.5.27] - 2026-02-10 (功能增强与体验优化合集)

### ✨ 新增功能 (New Features)

- **知识库 (Knowledge Base)**:
  - **解析中止**: 新增文件解析任务中止功能，支持在后台解析阶段随时中断任务。
  - **状态管理**: 引入 `cancelled` 状态，完善任务生命周期管理，支持取消后的清理建议。
  - **上传修复**: 修复文件上传参数构造问题，确保 `base_id` 正确传递。

- **即时通讯 (IM)**:
  - **消息回执**: 新增 `im_message_sent` 事件，即使推送消息成功状态给发送者。
  - **体验优化**: 优化消息推送逻辑（排除发送者自身），完善自动滚动 (`scrollToBottom`) 体验。

- **视频模块 (Video Course)**:
  - **上传流程重构**: 实现“先上传后保存”机制，静默创建章节并上传视频，避免保存失败导致视频丢失。
  - **进度反馈**: 优化上传进度条显示，完善取消上传时的资源清理逻辑。

- **Markdown 文档**:
  - **智能标题**: 新增自动获取文档标题功能，提升文档管理效率。
  - **布局优化**: 强制编辑页面在左侧显示，符合用户操作习惯。

- **反馈系统 (Feedback)**:
  - **实时通知**: 新增基于 WebSocket 的新反馈实时通知功能，管理员可即时收到反馈提醒。

### 🐛 Bug 修复 (Bug Fixes)

- **SQL 安全**:
  - 放宽 `is_safe_column_name` 校验规则，允许列名包含中文字符，修复数据分析模块的误报问题。

- **日程管理 (Schedule)**:
  - 修复 `selectinload` 未定义导致的 `NameError`，确保日程提醒关联数据正确加载。

- **系统备份 (Backup)**:
  - **策略调整**: 将 `analysis.db` 从文件备份迁移至数据库备份策略，解决文件锁定导致的备份失败问题。

- **公告模块 (Announcement)**:
  - 修复新发布公告在列表中不显示的问题，确保信息发布即时可见。

### 🎨 界面优化 (UI Improvements)

- **嫌疑人详情**: 优化列表列宽分配，增加文本截断与 Hover 显示，提升信息展示密度。
- **语义搜索**: 紧凑化欢迎页布局，压缩间距，确保核心信息在一屏内完整展示。

---

## [v2.5.26] - 2026-02-09 (安全审计修复：暴力破解防护 / 加密增强 / 隔离加固)

### 🛡️ P0 高危安全修复

- **登录暴力破解防护增强** (`routers/auth.py`):
  - 登录速率限制从 30 次/分钟收紧至 **10 次/分钟**（注册和改密收紧至 5 次/分钟）
  - 新增**账户级别锁定机制**：同一账户连续登录失败达到阈值（默认 5 次）后**锁定 5 分钟**，基于 Redis 计数，防分布式 IP 暴力破解
  - 登录成功后自动清除失败计数
  - IP 获取从 `request.client.host` 改为安全版 `get_client_ip()`（支持反向代理、含格式验证）
  - 失败日志不再记录用户名明文，防止日志泄露

- **备份加密密钥派生增强** (`utils/backup.py`):
  - 加密和解密的密钥派生从裸 `SHA-256` 替换为 **PBKDF2-HMAC-SHA256（480,000 次迭代）**
  - 防止攻击者获取加密备份后通过 GPU 高速暴力破解密码（SHA-256 速率约数十亿次/秒，PBKDF2 降至数百次/秒）
  - 加密和解密两侧使用相同的盐值和迭代参数，确保兼容性

- **默认管理员密码生产告警升级** (`core/config.py`):
  - 非 debug 模式下使用默认密码 `admin123` 启动时，日志级别从 `warning` 升级为 **`critical`**
  - debug 模式下保留 `warning` 级别提醒

### 🔒 P1 中高危安全修复

- **IM 消息加密盐值修复** (`modules/im/im_services.py`):
  - PBKDF2 盐值从**硬编码固定字符串** `b'im_message_encryption_salt'` 改为从 **JWT Secret 前 16 字节动态派生**
  - 环境变量密钥的 fallback 路径同步修复，使用密钥自身前 16 字节作为盐值
  - 消除了"JWT 密钥泄露 = 所有实例共享同一加密密钥"的风险

- **审计日志注入防护** (`core/audit_utils.py`):
  - `AuditLogger.get_client_ip()` 委托给 `utils.request.get_client_ip()`（含 IP 格式正则验证、长度限制 45 字符）
  - 消除了通过 `X-Forwarded-For: <script>alert(1)</script>` 向审计日志注入恶意内容的风险

- **快传传输码安全增强** (`modules/transfer/transfer_services.py`):
  - 随机数生成从 `random.randint`（Mersenne Twister 伪随机）替换为 **`secrets.randbelow`（密码学安全随机）**
  - 传输码长度从 6 位扩展到 **8 位**（组合空间从 90 万扩大到 **9000 万**，暴力破解难度提升 100 倍）

- **ETL 窗口函数隔离连接** (`modules/analysis/analysis_etl_service.py`):
  - `_execute_window` 从全局 DuckDB 实例改为**隔离的内存连接** (`duckdb.connect(":memory:")`)
  - 与 `_execute_sql` 保持一致，防止用户通过精心构造的字段名跨数据集访问其他表
  - 隔离连接在 `finally` 块中关闭，确保资源释放

### ⚡ P2 性能优化与安全增强

- **ETL 缓存大小估算修正** (`modules/analysis/analysis_etl_service.py`):
  - `sys.getsizeof(df)` + 固定 8 字节/列估算 替换为 **`df.memory_usage(deep=True).sum()`**
  - 修复字符串列等变长数据的内存占用被低估 5-10 倍的问题，使 LRU 淘汰机制正确生效

- **数据建模大数据集采样保护** (`modules/analysis/analysis_modeling_service.py`):
  - `get_summary` 接口对超过 **10 万行**的数据集自动添加 `USING SAMPLE 100000 ROWS`
  - 防止描述性统计计算将数百万行全量加载到内存导致 OOM

- **LIKE 通配符转义补全** (`modules/analysis/analysis_router.py`):
  - 搜索条件和 like/notlike 筛选的 3 处 LIKE 子句补全**反斜杠转义**（`\` → `\\`）
  - 修复用户输入包含反斜杠时可能导致 DuckDB 语法错误或意外转义下一个字符的问题

- **备份删除路径遍历防护** (`utils/backup.py`):
  - `delete_backup` 新增 `Path.resolve()` + `relative_to()` 路径安全校验
  - 确保删除路径必须在备份目录内，阻止通过路径遍历删除任意文件

### 📊 统计
- **测试验证**: 807 个测试全部通过 (806 passed, 1 skipped)
- **修复分布**: P0 高危 3 项 / P1 中高危 4 项 / P2 增强 4 项

---

## [v2.5.25] - 2026-02-09 (全面安全加固 + 性能优化 + 全量测试覆盖)

### 🛡️ 安全加固 (Security Hardening)

- **密码安全**:
  - `core/security.py`: `hash_password()`/`verify_password()` 新增 SHA-256 **预哈希**（`_prehash_password()`），解决 bcrypt 72 字节截断问题；保留旧版密码的向后兼容验证
  - `core/config.py`: 生产环境使用默认 `JWT_SECRET` 时直接**拒绝启动**（原仅 warning）；新增默认管理员密码告警
  - `routers/system_settings.py`: 新增系统设置边界校验：`jwt_expire_minutes`(5~1440)、`rate_limit_requests`(1~100000)、`password_min_length`(4~128)

- **SQL 注入全面防护**:
  - `analysis/analysis_compare_service.py`: 所有表名/列名/join_keys 通过 `is_safe_table_name()`、`escape_sql_identifier()` 验证和转义
  - `analysis/analysis_modeling_service.py`: 新增 `_validate_sql_readonly()` 白名单验证（仅允许 SELECT/WITH）；黑名单禁止 DROP/DELETE/CREATE 等；**禁止 DuckDB 文件读取函数**（`read_csv`/`read_parquet` 等防任意文件读取）；聚合函数白名单限制
  - `analysis/analysis_import_service.py`: DuckDB `CREATE TABLE` 使用标识符转义
  - `analysis/analysis_router.py`: 排序方向白名单 (asc/desc)；LIKE 通配符转义
  - `core/database.py`: `SET time_zone` 时区值新增正则校验，防止时区配置注入
  - `routers/user.py`: LIKE 通配符转义；**移除公开搜索中的手机号搜索**防隐私泄露
  - `routers/storage.py`、`filemanager/filemanager_services.py`: 关键词搜索 LIKE 通配符转义

- **SSRF 防护**:
  - `analysis/analysis_import_service.py`: 新增 `_validate_connection_url()`：协议白名单（仅 mysql/postgresql/mssql）；禁止 localhost/127.0.0.1 等本地地址；**禁止内网 IP 段**；**DNS Rebinding 检查**（解析主机名后验证所有 IP）；新增 `_validate_query_readonly()` 验证外部查询仅为 SELECT
  - 外部数据库连接 URL **不再明文存储**（保存为 `***已脱敏***`）

- **路径遍历防护**:
  - `utils/storage.py`: `_is_safe_path()` 改用 `Path.relative_to()` 替代 `str.startswith()`，避免前缀匹配绕过
  - `utils/backup.py`: tar 解压安全过滤——**拒绝符号链接/硬链接/设备文件**；路径校验改用 `relative_to()`
  - `routers/backup.py`: 备份下载新增 `resolve()` 路径校验
  - `filemanager/filemanager_services.py`: 新增 `_validate_name()` 禁止 `../\x00` 及 Windows 保留名（CON/PRN/NUL）
  - `filemanager/filemanager_router.py`: 文件夹上传路径过滤 `..`；上传文件名使用 `basename()` + 正则清洗；SVG 强制以 `attachment` 下载防 XSS

- **CSRF 安全**:
  - `core/csrf.py`: 新增 `verify_and_consume_csrf_token()` **原子化验证+消耗**，解决 TOCTOU 竞态条件；验证后立即删除防重放攻击；内存 token 上限 10000 防 DoS

- **CORS 安全**:
  - `main.py`: `allow_origins=["*"]` 时自动**禁用 `allow_credentials`**，防任意网站劫持会话

- **认证与权限**:
  - `core/security.py`: 权限不足错误信息不再泄露具体权限名；权限同步失败返回 503 而非静默降级
  - `core/middleware.py`: 请求 ID **始终服务端生成**（防伪造和日志注入）
  - `utils/request.py`: 新增 `_sanitize_ip()` IP 清洗函数，正则验证格式，长度限制 45 字符
  - `routers/notification.py`: 仅 admin/manager 可向其他用户发送通知
  - `routers/market.py`: 模块包上传新增 **100MB 大小限制**

- **信息泄露修复**:
  - `routers/backup.py`、`storage.py`、`user.py`、`filemanager_router.py`: 500 错误不再暴露内部异常信息
  - `core/lifespan.py`: 管理员创建日志不再输出密码明文
  - `main.py`: 生产环境关闭 `/api/docs`、`/api/redoc`、`/api/openapi.json`

- **前端 XSS 全面消除（100+ 处）**:
  - 消除了 `analysis_smart_table.js`、`dock.js`、`pdf.js`、`exam.js`、`lm_cleaner.js`、`notes.js`、`im.js`、`topbar.js`、`office_viewer.js`、`map.js`、`course.js`、`announcement.js`、`blog.js`、`users.js`、`profile.js` 等 **20+ 个文件中的 100+ 处 `onclick`/`onchange`/`oninput` 内联事件处理器**，统一改为 `data-*` 属性 + 事件委托模式

- **前端 Token 保护**:
  - `core/component.js`: 新增 `Utils.withToken(url)` 统一工具方法，替代 12+ 个文件中硬编码的 `token=xxx` URL 拼接
  - `core/websocket.js`: Token 从 URL 查询参数改为 **WebSocket Subprotocol** (`['bearer', token]`) 传递
  - `filemanager.js`: 文件下载从 iframe 改为 `Api.download()` + Blob URL

- **前端代码注入防护**:
  - `analysis_smart_table.js`: `new Function()` 替换为安全的 **Shunting-yard（调度场）算法**解析器，完全杜绝代码注入

### 🚀 性能优化 (Performance Optimizations)

- **后端**:
  - `core/security.py`: Fernet 实例缓存，仅在 secret 变化时重建
  - `core/cache.py`: `clear_pattern()` 改为每 100 个 key 批量删除，减少网络往返
  - `core/loader.py`: 拓扑排序改用 `collections.deque`（`popleft()` O(1)）
  - `utils/backup.py`: 数据库导出改为**分批读取**（每批 5000 行），防大表 OOM
  - `filemanager/filemanager_services.py`: `browse_directory()` 文件计数改为批量查询，解决 N+1 问题；`_get_breadcrumbs()` 批量查询祖先
  - `analysis/analysis_router.py`: 大数据集查询使用默认线程池，替代每次创建新 `ThreadPoolExecutor`
  - `analysis/analysis_duckdb_service.py`: DuckDB 连接使用**双重检查锁定** + `threading.RLock` 线程安全

- **前端**:
  - `dock.js`: 图标映射表**静态缓存**；自动隐藏 `mousemove` 使用 **`requestAnimationFrame` 节流**
  - `filemanager.js`: 初始化改为 `Promise.all()` **并行加载**
  - `app.js`: 桌面时钟更新间隔从 1 秒改为 **15 秒**
  - `im_components.js`、`analysis_sql.js`: DOM 渲染改用 **`DocumentFragment` 批量插入**
  - `analysis_smart_table.js`: 事件从逐元素绑定改为**单一容器委托**

### 🔧 功能增强 (Enhancements)

- **WebSocket 认证扩展**: `routers/websocket.py` 新增**子协议方式** + **HttpOnly Cookie** 方式，共支持三种认证方式
- **智能限流**: `core/rate_limit.py` 新增 `_get_rate_limit_key()`，优先使用**已认证用户 ID** 作为限流键（减少 NAT/共享 IP 误伤）；默认限流调整为 200 req/60s
- **文件管理器搜索**: `filemanager/filemanager_services.py` `search()` 重写为**全局搜索**（跨所有文件夹）
- **权限同步重构**: `core/security.py` 提取 `_sync_user_permissions()` 共享函数，消除 `get_current_user`/`get_optional_user` 中约 40 行重复代码
- **中间件重构**: `core/middleware.py` 提取 `_handle_no_response_error()` 公共方法，消除 5 个中间件中的重复处理
- **登录权限修复**: `routers/auth.py` 登录响应返回**完整权限**（直接权限 + 角色权限），修复之前不含角色权限的问题

### 🐛 Bug 修复 (Bug Fixes)

- **后端**:
  - `core/security.py`: 移除模块级 `settings` 缓存，修复 `reload_settings()` 后引用旧对象
  - `core/cache.py`: 序列化统一 `json.dumps()`，修复字符串 `"true"` 被误解析为布尔值
  - `core/events.py`: `emit()` 同步路径缺少 `_trim_history()` 调用，修复事件历史无限增长
  - `core/loader.py`: 模块加载失败时清理 `sys.modules` 残留；`unload_module()` 调用 `on_disable` 而非 `on_uninstall`
  - `filemanager/filemanager_services.py`: `delete_folder()` 仅顶层 `commit()`，修复递归多次 commit 事务不一致
  - `analysis/analysis_compare_service.py`: 采样临时表清理放入 `finally`；临时表名加 UUID 避免并发冲突

- **前端**:
  - `core/api.js`: **Token 刷新竞态条件修复**——包装为单个 Promise，避免多个 401 各自触发独立 refresh
  - `core/api.js`: `upload()` 方法新增 401 自动刷新 + 重试逻辑
  - `dock.js`: `destroy()` 新增清理 auto-hide 事件监听器和定时器，修复**内存泄漏**
  - `app.js`: 时钟定时器先清理再创建，防止多个定时器并行；404 页面改为 `Toast.warning()` 非阻塞提示
  - `filemanager.js`: 选择项 ID 解析 `split('-')` 改为 `indexOf('-') + substring()`，修复 ID 含连字符时解析错误
  - `datalens.js`: 图片加载失败新增错误占位图兜底

### ✅ 测试体系建设 (Test Suite)

- **核心系统测试** (10 个新建/增强文件, 272 个测试):
  - 新建: `test_text_utils.py`、`test_timezone_utils.py`、`test_sql_safety.py`、`test_request_utils.py`、`test_storage_utils.py`、`test_auth_helpers.py`、`test_main_app.py`
  - 增强: `test_security.py`（令牌对/密钥轮换/加解密/权限检查）、`test_middleware.py`（脱敏/统计/审计）、`test_cache.py`（自增减/模式清除/重试）

- **功能模块测试** (19 个模块全覆盖, 416 个测试):
  - 从占位符重写: `exam`（23 测试）
  - 从极少量扩展: `map`（1→17）、`ai`（11→43）
  - 新增 API 路由测试: `blog`、`notes`、`schedule`、`course`、`feedback`、`video`、`vault`、`transfer`、`im`、`album`、`datalens`、`filemanager`、`lm_cleaner`、`knowledge`、`markdown`、`pdf`、`ocr`

- **测试基础设施修复**: `test_conftest.py` 速率限制 mock 参数修复；`test_rate_limit.py` MagicMock headers 修复；`test_system_settings.py` 校验范围修复；`test_analysis.py`/`test_modeling.py` 错误消息 regex 修复

### 📊 统计
- **变更规模**: 112 个文件, +6461/-3984 行
- **测试总计**: 843 个用例 (842 passed, 1 skipped)
- **执行耗时**: 约 3 分钟 (虚拟环境, SQLite 内存数据库)

---

## [v2.5.24] - 2026-02-08 (Markdown XSS 修复与全站 CSRF 兼容性增强)

### 🛡️ 安全漏洞修复 (Security Fixes)
- **Markdown WYSIWYG 编辑器 XSS 修复**:
  - 修复了编辑模式下**行内格式化处理**（粗体、斜体、删除线、行内代码、高亮等标记）的 XSS 漏洞，对正则匹配捕获的用户内容应用 `escapeHtml()` 转义。
  - 修复了**列表项内容**渲染时的 XSS 风险，确保无序/有序列表文本经过安全转义。
  - 修复了**任务列表项内容**渲染时的 XSS 风险，确保待办事项文本经过安全转义。

- **CSRF 兼容性与上传修复**:
  - **全站上传功能修复**: 修复了即时通讯 (IM)、地图 (Map)、数据透镜 (DataLens)、相册 (Album)、文件管理 (FileManager) 等模块中直接使用 `fetch` 或 `XMLHttpRequest` 进行文件上传时，因缺失 `X-CSRF-Token` 头导致的 `403 Forbidden` 错误。
  - **统一请求头标准化**: 全局审查并修正了前端所有直接调用 API 的场景，确保所有数据提交请求均携带正确的认证凭证与 CSRF Token。
  - **Office Viewer 优化**: 修复了 Office 文档预览组件 (`office_viewer.js`) 的加载问题。
  - **API 核心增强**: 优化了 `frontend/js/core/api.js`，增强了对上传请求的错误处理与凭证管理。

### 🚀 功能增强 (Enhancements)
- **全局搜索 (Spotlight) 深度增强**:
  - **搜索范围扩展**: 快捷搜索现在支持 **用户 (Users)**、**笔记 (Notes)**、**知识库 (Knowledge)**、**相册 (Album)**、**视频 (Video)**、**系统公告 (Announcements)**、**博客 (Blog)**、**应用市场 (Market)** 和**数据透镜 (DataLens)** 的全面动态搜索。
  - **管理员深度审计**: 为管理员和经理增加了 **审计日志 (Audit Logs)** 的直接搜索功能，可快速查找系统动作与异常记录。
  - **极速响应优化**: 针对知识库搜索引入了 `mode=quick` 模式，在快捷搜索场景下仅执行关键词检索，避开了沉重的向量模型 (Embedding/CLIP/Rerank) 加载与计算开销。
  - **并发搜索逻辑**: 引入 `Promise.allSettled` 并发机制，同步发起 10+ 个模块的搜索请求，各模块结果异步回填，极大提升了全搜响应体验。
  - **全局 API 标准化**: 将多模块接口（Album, Video, Lens, Im, Announcement）统合进核心 API 模块并注册到 window 全局，确保 Spotlight 能够随时发起稳定的 API 调用。
  - **静态索引更新**: 重新整理并增加了 20+ 个常用应用与设置的快速跳转入口。

### 🐛 Bug 修复 (Bug Fixes)
- **知识库模块修复**:
  - **搜索延迟修复**: 修复了快捷搜索时触发知识库全量混合搜索，导致服务器因加载大型 AI 模型（如 CLIP, Reranker）而产生 20s+ 延迟及“慢请求”告警的问题。
  - **过滤条件修复**: 修复了向量数据库 (ChromaDB) 在执行空过滤条件或多重条件组合搜索时抛出 `ValueError` 的问题，优化了过滤子句的原子化构造逻辑。
- **Spotlight 路由修复**:
  - 修正了笔记搜索接口的跳转路径 ( `/notes` -> `/notes/notes`)，修复了搜索笔记返回 404 的问题。
- **相册模块修复**:
  - 修复了照片查看器在打开后，由于 HTML 结构缺失导致的照片无法居中（严重偏下）的问题。
  - 修复了相册后台路由中 `Settings` 属性名引用错误 (`storage_path` -> `upload_dir`) 导致的 API 调用异常。
- **全局模块修复**:
  - 对视频模块、知识库模块中存在的相同 `Settings` 属性名引用错误进行了同步修复，确保文件读取安全性路径校验正常工作。

### 🔍 安全审计 (Security Audit)
- **前端 XSS 全面扫描**: 对 `frontend/js` 目录下所有使用 `innerHTML` 和 `insertAdjacentHTML` 的 JavaScript 文件进行了全面安全审计。
- **已审查页面模块**: 
  - `im.js`、`im_components.js` - 即时通讯模块，消息渲染、用户昵称、群设置等内容安全转义
  - `map.js` - 地图模块，文件名、地点名称、轨迹信息安全转义
  - `vault.js` - 密码箱模块，错误信息、文件名安全转义
  - `filemanager.js` - 文件管理模块，文件/文件夹名、图标渲染安全处理
  - `market.js` - 应用市场模块，应用名称、描述安全转义
  - `exam.js` - 考试模块，题目、选项、成绩等内容安全转义
  - `analysis_modeling.js` - 数据分析ETL模块，节点标签、字段名安全转义
  - `messages.js` - 通知中心模块，通知标题、内容安全转义
  - `ai.js` - AI助手模块，会话标题、Markdown渲染安全处理（含URL协议检查）
  - `album.js` - 相册模块，相册/照片名称、URL安全转义
  - `video.js` - 视频模块，视频标题、URL安全转义
  - `users.js` - 用户管理模块，用户数据安全转义
  - `pdf.js` - PDF工具模块，文件名、标题安全转义
  - `lm_cleaner.js` - LM水印清除模块，历史记录文件名安全转义
  - `course.js` - 课程学习模块，课程标题、描述安全转义
  - `backup.js` - 备份模块，文件名、描述、错误信息安全转义
  - `knowledge.js` - 知识库模块，知识库名称、描述安全转义
  - `markdown.js` - Markdown列表模块，文档标题、摘要安全转义
- **已审查组件模块**:
  - `topbar.js` - 顶部栏组件，用户名、消息标题、公告标题安全转义
  - `start_menu.js` - 开始菜单组件，菜单项标题、路径、图标安全转义
  - `spotlight.js` - 全局搜索组件，搜索结果图标、标题、描述安全转义
  - `window_manager.js` - 窗口管理器，窗口标题安全转义
- **已审查核心模块**:
  - `datalens_hub.js` - 数据透镜Hub，分类、视图卡片内容安全转义
- **安全状态**: 除 `markdown_wysiwyg.js` 中发现并修复的漏洞外，其他所有模块均符合前端安全规范，一致使用 `Utils.escapeHtml()` 和 `Utils.escapeAttr()` 进行安全转义。

---

## [v2.5.23] - 2026-02-08 (安全与性能优化：HttpOnly Cookie / CSP / SQL 校验)

### 🔒 安全与性能审计 (Security & Performance Audit)
- **审计报告**: 新增 `docs/安全与性能审计报告.md`，对后端、前端、配置与部署进行安全与性能审计，并实施相应修复与优化。
- **登录请求体长度限制**: `schemas/auth.py` 中 `UserLogin` 增加 `username` / `password` 的 `min_length`、`max_length`（1–128 / 1–256），防止超大请求体 DoS 与无意义暴力尝试。
- **生产环境配置提醒**: `core/config.py` 在 `debug=False` 时对默认 `JWT_SECRET`、`allow_origins=["*"]` 打安全警告日志；`.env.example` 补充 CORS、HttpOnly Cookie 等配置说明。

### 🍪 Token 存储改为 HttpOnly Cookie (防 XSS 窃取)
- **后端**:
  - 新增配置 `AUTH_USE_HTTPONLY_COOKIE`（默认 `true`）：登录/刷新时通过 `Set-Cookie` 下发 `access_token`、`refresh_token`（HttpOnly、Secure=非 debug、SameSite=Lax）；登出时清除上述 Cookie。
  - `core/security.py`：`get_current_user` / `get_optional_user` 优先从 Cookie 读取 token，其次 Authorization 头、Query。
  - `utils/auth_helpers.py`：`get_user_from_token` / `get_admin_from_token` 支持从 `Request` 的 Cookie 读取，供下载、导出等接口使用。
  - 刷新接口支持从 Cookie 或 body 读取 `refresh_token`；`/system/init` 与登录响应中返回 `use_http_only_cookie`。
- **前端**:
  - `Config.useHttpOnlyCookie` 由 init/登录响应设置；`Store.setAuth` 在 HttpOnly 模式下不再将 token 写入 localStorage；`Store.init` 在仅有 user 无 token 时仍视为已登录。
  - `api.js`：所有请求增加 `credentials: 'include'`；HttpOnly 模式下不读写 localStorage 的 token，401 时使用 Cookie 调用刷新接口。
  - `login.js`：登录成功后根据 `use_http_only_cookie` 设置 Config 并传入 `refresh_token`。
- **路由**：`storage`、`import_export`、`backup` 等使用 `get_user_from_token` 的下载/导出接口增加 `Request` 参数，改为 `get_user_from_token(request, token)`，以支持 Cookie 鉴权。

### 🛡️ Datalens / 分析模块 SQL 安全 (防注入)
- **datalens_services.py 流式导出路径**:
  - 对 `query_config` 中的表名使用 `utils.sql_safety.is_safe_table_name` 校验，列名（含 dict 的 `field`）使用 `is_safe_column_name` 校验。
  - 构建后的 SQL 使用 `validate_sql` 校验；`QueryType.SQL` 时对原始 SQL 同样做 `validate_sql`，避免 SQL 注入。

### 📋 Content-Security-Policy (CSP)
- **SecurityHeadersMiddleware**：增加 `Content-Security-Policy` 响应头（`default-src 'self'`；script/style 支持同源与 `unsafe-inline`；img/font/connect 按 SPA 需求配置；`frame-ancestors 'none'`；`base-uri 'self'`）。

### 📝 登录失败信息 (防枚举)
- **routers/auth.py**：在登录失败分支增加注释，明确统一返回“用户名或密码错误”、不区分用户是否存在，符合防用户名枚举最佳实践（逻辑未改）。

### 📚 文档 (Documentation)
- **安全与性能审计报告**：将上述四项（HttpOnly Cookie、Datalens SQL 校验、CSP、登录防枚举）列为“已完成的四项优化”，并更新报告中的建议与可选优化说明。

---

## [v2.5.22] - 2026-02-08 (考试模块阅卷增强与数据工具组件)

### 📝 在线考试模块增强 (Exam Module Enhancement)
- **阅卷防作弊日志**:
  - **异常行为记录**: 在阅卷详情页新增“异常行为记录”面板，直观展示考生在考试过程中的切屏、离开窗口等可疑行为。
  - **时间轴展示**: 详细列出每次异常行为的发生时间、类型及累计次数，辅助教师进行公平判卷。
  - **视觉警示**: 采用警示色突出显示异常记录，确保阅卷人不会忽略潜在的作弊行为。
- **考试体验增强**:
  - **倒计时智能提醒**: 考试剩余 5 分钟和 1 分钟时自动弹出醒目的时间提醒弹窗，并播放提示音效，防止考生超时。
  - **键盘快捷键导航**: 支持使用 `↑/↓/←/→` 键快速切换题目，`Ctrl+Enter` 一键提交试卷。
  - **断点续做优化**: 进入考试模块时自动检测未完成的考试，弹窗提示可一键恢复上次进度。

### 🧩 新增组件 (New Components)
- **数据工具组件 (DataTools)**:
  - **多字段筛选面板**: 提供可视化的数据筛选 UI，支持等于、包含、大于等 10 种操作符。
  - **多字段排序面板**: 支持按优先级添加多个排序字段，可设置升序/降序。
  - **Mixin 机制**: 提供 `createMixin()` 方法，可快速为任意 Component 组件集成筛选排序能力。
  - **应用范围**: 已集成至数据透镜 (DataLens)、数据分析 (Analysis) 等模块，提供统一的数据操作体验。

### 🛠️ 核心工具库升级 (Core Utilities Upgrade)
- **下载文件名编码规范化 (RFC 5987)**:
  - 在 `filemanager_router.py` 中新增 `encode_filename_for_header` 工具函数。
  - 自动检测文件名是否包含非 ASCII 字符（如中文），并采用 RFC 5987 标准 (`filename*=UTF-8''...`) 进行编码。
  - 彻底解决了在不同浏览器（尤其是老旧浏览器或特殊环境）下下载中文文件时文件名乱码或被截断的问题。
- **认证辅助工具标准化**:
  - 新增 `backend/utils/auth_helpers.py` 模块。
  - 封装 `get_user_from_token` 和 `get_admin_from_token` 函数，专门用于处理 Query 参数中的 Token 验证。
  - 统一了 WebSocket 连接、文件下载、导出等无法通过 Header 传递 Token 场景下的认证逻辑，提升了代码复用性与安全性。
- **Modal 组件静态方法扩展**:
  - 新增 `Modal.form(options)`: 支持通过配置快速生成包含输入框、下拉框、复选框的表单模态框，简化业务代码。
  - 新增 `Modal.select(options)`: 提供标准的列表选择对话框，支持异步回调。
  - **Bug 修复**: 修复 `Modal.show` 静态方法返回值类型错误，解决调用后 `modal.query` 不存在导致的报错问题。

### 🛡️ 安全加固 (Security Hardening)
- **考试模块 XSS 深度防护**:
  - 对考试模块所有动态内容（题目、选项、用户答案、评语等）统一应用 `Utils.escapeHtml` 转义。
  - 对题库名称、试卷标题、排名数据等 HTML 属性应用 `Utils.escapeAttr` 防护。
  - 阅卷详情页的作弊日志内容全量转义，防止恶意考生通过异常行为描述注入脚本。
- **通知系统 XSS 修复**:
  - 修复通知列表中标题、内容、发送者名称的未转义渲染问题。
  - 统一使用 `Utils.escapeHtml` 处理所有用户生成内容。

---

## [v2.5.21] - 2026-02-08 (核心组件交互优化与备份模块修复)

### 🛠️ 核心组件优化 (Core Component Improvements)
- **Modal 全局框架构升级**:
  - **幂等关闭机制**: 引入 `_closing` 状态锁，解决模态框在快速连续点击或动画未完成时关闭导致的 DOM 残留与逻辑异常。
  - **交互增强**: 新增支持 `Enter` 键在输入框内直接提交表单；在异步处理 (`onConfirm`) 期间自动锁定所有页脚按钮，防止重复提交。
  - **作用域查询**: 扩展 `modal.query()` 方法，支持在单个模态框 DOM 作用域内查找元素，从根源上规避全局 ID 冲突引发的交互失效。
  - **内存管理优化**: 完善了 ESC 键及全局事件监听器的回收机制，确保组件卸载后无残留。

### 🐞 Bug 修复 (Bug Fixes)
- **数据备份 (Backup) 模块修复**:
  - **模态框关闭异常**: 修复了“自动备份计划”弹窗因事件绑定冲突导致的点击确认/取消后无法正确关闭的问题。
  - **事件绑定重构**: 将模态框相关的表单初始化逻辑从全局 `setTimeout` 延迟异步，改为基于模态框实例的即时局部绑定。
  - **恢复流程体验优化**: 修正了数据恢复操作的异步处理，现在在恢复过程中弹窗将保持展示加载状态，直到接收到后端反馈。

---

## [v2.5.20] - 2026-02-08 (DataLens 与地图模块安全深度加固)

### 🛡️ 安全深度修复 (Security Hardening)
- **数据分析 (DataLens) XSS 漏洞修复**:
  - **数据建模 (analysis_modeling.js)**: 对 ETL 节点标签、节点摘要信息以及实时预览表格中的表头和单元格数据进行了 HTML 转义处理，防止恶意脚本通过表名或自定义内容注入。
  - **BI 仪表盘 (analysis_bi.js)**: 修复了图表加载失败时错误消息的未转义展示问题。
  - **智能表格 (analysis_smart_table.js)**: 对字段标签、选项值以及 `data-label` 等 HTML 属性应用了安全转义，确保用户自定义表单的安全性。
- **地图模块 (map.js) 安全加固**:
  - **规范化转义**: 将 Marker 弹出框中误用的全局 `escape` 函数统一替换为项目标准的 `Utils.escapeHtml`。
  - **导出安全**: 在生成 GPX 轨迹文件时，对数据集名称进行了严格的 XML 实体转义。
  - **属性安全**: 对地图切片 URL 和手动数据源输入的 `value` 属性应用了 `Utils.escapeAttr`。
- **核心工具类 (lazy_loader.js) 优化**:
  - **全属性转义**: 强化了 `createLazyImg` 的安全性，不再仅限于 `alt` 属性，而是对 `src`、`class`、`width`、`height` 及 `fallback` 等所有动态属性统一应用 `Utils.escapeAttr` 转义，彻底杜绝属性级注入风险。

---

## [v2.5.19] - 2026-02-07 (前端重构与安全规范整合)

### 🏗️ 架构重构 (Refactoring)
- **文件命名规范化**: 将 `frontend/js/pages/im/components.js` 重命名为 `frontend/js/pages/im/im_components.js`，避免文件名冲突并明确文件功能。
- **引用路径更新**: 同步更新 `loader.js` (ResourceLoader) 和 `index.html` 中的引用路径，确保 IM 模块资源加载正常。

### 🛡️ 安全修复 (Security Fixes)
- **全站 XSS 漏洞修复 (Cross-Site Scripting)**: 对全站核心模块进行了全面的 XSS 安全审计与修复，确保所有用户输入均经过严格转义与校验。
  - **即时通讯 (IM)**: 修复消息内容、群组名称、成员昵称及系统通知中的 HTML 注入风险。
  - **视频模块 (Video)**: 修复视频标题、描述及播放列表名称的未转义渲染问题。
  - **博客/公告 (Blog/Announcement)**: 增强 Markdown 解析器的 HTML 过滤白名单，防止恶意脚本注入。
  - **用户管理 (Users)**: 修复用户列表搜索结果高亮 (`<mark>`) 时的 DOM 拼接漏洞。
  - **文件管理 (FileManager)**: 修复文件名、文件路径在列表视图及属性弹窗中的 XSS 隐患。
  - **富文本编辑器**: 升级 `MarkdownWysiwyg` 组件，增加对 `javascript:` 伪协议链接的拦截与过滤。
  - **其他修复**: 覆盖 `start_menu.js` (应用名称), `system.js` (日志详情), `map.js` (地点标记) 等多处潜在风险点。

### 📚 文档更新 (Documentation Updates)
- **规范整合 (Standard Consolidated)**: 将独立的 `前端开发安全规范.md` 完整整合至 `开发规范.md` 的 `5.6 前端安全规范` 章节。
  - 核心原则：明确“永不信任用户输入”的安全基线。
  - 渲染规范：强制要求使用 `Utils.escapeHtml` 转义，严禁裸写 `innerHTML`。
  - 属性安全：新增对 `href` 和 `src` 伪协议攻击 (`javascript:`) 的防御指南。
  - 审计工具：统一转义工具函数标准。

---

## [v2.5.18] - 2026-02-07 (异常处理规范化)

### 🔧 代码质量提升 (Code Quality)
- **空异常捕获全量修复**: 根据开发规范，将所有裸 `except:` 修改为捕获具体异常类型或 `Exception`，共修复 29 处：
  - `backup_executor.py`: 加密文件大小获取失败时记录日志
  - `storage.py`: 临时文件清理失败时记录日志（新增 logger 实例）
  - `backup.py`: 时间格式验证改为捕获 `(ValueError, TypeError)`
  - `pdf_services.py`: 文件路径获取改为捕获 `ValueError`
  - `map_router.py`: CSV 编码检测改为捕获 `(UnicodeDecodeError, pd.errors.ParserError)`
  - `im_router.py`: 文件 MIME 类型检测改为捕获 `(ImportError, Exception)` 并记录日志
  - `datalens_router.py`: 路径解析改为捕获 `(ValueError, OSError)` 并记录日志
  - `ai_service.py`: JSON 解析改为捕获 `json.JSONDecodeError`，数据集列获取改为记录日志
  - `analysis_router.py`: 排序参数解析改为捕获 `json.JSONDecodeError`
  - `analysis_cleaning_service.py`: 数值转换改为捕获 `(ValueError, TypeError)`
  - `analysis_etl_service.py`: 多处数值转换和资源清理改为具体异常类型
  - `analysis_compare_service.py`: 临时表清理改为记录日志
  - `security.py`: 可选用户权限获取改为记录日志
  - `test_conftest.py`: 测试资源清理改为捕获 `Exception`
  - `create_module.py`: 编码设置和文件读取改为捕获具体异常类型

### 📋 规范遵循 (Standards Compliance)
- 所有异常捕获现在符合开发规范要求，使用具体异常类型或 `Exception` 并记录日志
- 提高了代码的可调试性和健壮性，避免隐藏潜在错误

### 🏗️ 时区一致性加固 (Timezone Consistency)
- **统一东八区时间**: 继续修复多个模块中的 `datetime.now()` 调用，统一使用 `get_beijing_time()` 确保时区一致性：
  - `analysis_smart_table_service.py`: 数据集更新时间
  - `course_services.py`: 章节学习完成时间、课程完成时间
  - `ai_router.py`: AI 配置更新时间
  - `transfer_router.py`: WebSocket 消息时间戳
  - `scheduler.py`: 每日任务调度时间
  - `jwt_rotate.py`: JWT 密钥轮换时间戳、下次轮换时间计算（共 7 处）

### 🔒 安全漏洞修复 (Security Fixes)
- **tarfile 路径穿越防护**: 修复 `backup.py` 中 `tarfile.extractall()` 存在的路径穿越漏洞（CVE-2007-4559），添加成员路径安全过滤
- **SQL 注入防护**: 新增 `utils/sql_safety.py` 安全模块，提供表名/列名验证和转义函数
  - `is_safe_table_name()`: 验证标识符只包含字母、数字、下划线，并排除 SQL 关键字
  - `sanitize_table_name()`: 安全清理并返回带前缀的表名
  - `escape_sql_identifier()`: 转义 SQL 标识符
- **DuckDB 表名验证**: 在 `analysis_modeling_service.py` 和 `analysis_etl_service.py` 中添加用户输入的表名验证
- **公式计算安全性增强**: 优化 `analysis_smart_table_service.py` 中的 `safe_eval_dataframe_formula` 函数，增加 `engine='python'` 和 `parser='pandas'` 参数约束

### 📚 文档更新 (Documentation Updates)
- **开发规范安全增强**: 更新 `开发规范.md`，新增 `3.9 安全开发规范` 章节
  - 明确 **SQL 注入防护** 要求和工具使用
  - 增加 **路径穿越防护** 规范
  - 增加 **DuckDB 安全使用** 规范
  - 增加 **pandas.eval** 安全参数要求
  - 增加 **前端 XSS 防护** 规范

### ⚡ 代码优化 (Optimization)
- **LocalStorage 键名标准化**: 统一将分散在各模块的 `localStorage` 键名移至 `Config.storageKeys` 管理
  - 涉及模块：Market, Login, Map, Course, AI
  - 新增键名：`mapConfig`, `videoPlaybackSpeed`, `aiSessions`, `courseLastChapter` 等
- **脚本安全增强**: `delete_module.py` 引入 `utils.sql_safety` 模块进行模块ID校验

---

### 🐛 缺陷修复 (Bug Fixes)
- **文件下载内存优化**: 修复文件夹打包下载时将整个 ZIP 加载到内存的严重隐患，改为使用临时文件并在下载后自动清理，并限制最大打包大小为 2GB。
- **WebSocket 异常处理优化**: 修复客户端突然断开时导致的 `RuntimeError: Cannot call "send" once a close message has been sent` 错误，添加安全的连接状态检查和关闭方法。
- **调试代码清理**: 移除生产环境代码中残留的 `traceback.print_exc()` 调试输出。
- **API 健壮性**: 明确标记 AI 服务的同步接口为未实现状态，避免错误调用。
- **空异常捕获修复**: 修复 `im_websocket.py` 中违反开发规范的 `except: pass` 模式，改为记录 debug 级别日志，便于问题排查。
- **前端硬编码修复**: 修复 `knowledge.js` 中 `localStorage.getItem('token')` 硬编码问题，统一使用 `Config.storageKeys.token`。

### 🏗️ 时区一致性加固 (Timezone Consistency)
- **统一东八区时间**: 修复多个模块中的 `datetime.now()` 调用，统一使用 `get_beijing_time()` 确保时区一致性：
  - `transfer_services.py`: 会话过期判断、连接时间、完成时间
  - `filemanager_services.py`: 文件夹/文件更新时间
  - `datalens_services.py`: 数据源/分类/视图更新时间
  - `market.py`: 模块安装/更新时间

---

## [v2.5.16] - 2026-02-07 (API 修复与生命周期加固)

### 🐛 缺陷修复 (Bug Fixes)
- **用户 API 请求优化**: 修复 `NotificationCenter` 获取用户列表时 `size` 参数超过后端限制（100）导致的 `400 Bad Request` 错误，统一调整为 `100`。
- **AI 聊天安全修复**: 修复 AI 聊天流式接口缺失 `X-CSRF-Token` 导致的 `403 Forbidden` 错误。
- **地图组件异常修复**: 修复 `map.js` 中 `this.clearInterval` 未定义的错误，通过在 `Component` 基类实现受控定时器方法解决。
- **审计日志功能增强**: 
  - 系统日志页面增加 **用户 ID 筛选** 功能。
  - 增强关键词搜索，支持按 **用户名/昵称** 进行模糊匹配。
  - 实现日志表格中 **点击用户名快速筛选** 功能，优化管理体验。

### 🏗️ 架构加固 (Architecture Hardening)
- **受控生命周期扩展**: 在 `Component` 类中补充 `clearTimeout` 和 `clearInterval` 方法，完善受控定时器闭环管理。

---

## [v2.5.15] - 2026-02-07 (前端生命周期与内存管理优化)

### 🏗️ 前端生命周期与内存管理 (Frontend Lifecycle & Memory Management)
- **受控定时器系统 (Managed Timers)**: 全量重构 `IM`、`地图`、`数据分析`（含 BI、建模、图表生成器）模块。将所有 `setTimeout` 和 `setInterval` 替换为基类受控方法，确保在窗口关闭或组件销毁时自动清理，杜绝后台冗余运行。
- **全局事件受控绑定**: 将 `window` 和 `document` 的原生事件监听（如 `resize`、`click`）迁移至组件受控绑定机制（`addWindowEvent`/`addDocumentEvent`），实现销毁时的零残留清理，防止内存泄漏。
- **组件销毁逻辑加固**:
  - **ECharts 实例自动释放**: 优化 `AnalysisBIPage` 和 `AnalysisPage`，确保在销毁时自动调用 `chart.dispose()`。
  - **嵌套组件递归销毁**: 实现了子组件实例的显式销毁调用，确保复杂的 UI 结构在卸载时能释放所有资源。
  - **冗余代码清理**: 合并了 `AnalysisPage` 中重复定义的 `destroy` 方法，统一资源清理入口。

---

## [v2.5.14] - 2026-02-07 (安全加固与架构优化)

### 🛡️ 安全加固 (Security Hardening)
- **CSRF 防护默认开启**: 修改 `backend/core/config.py` 和 `.env` 模板，将 `CSRF_ENABLED` 默认设置为 `True`。
- **CSRF 模块性能重构**: 
  - **异步化支持**: 将 CSRF Token 生成与验证重构为异步函数，提升高并发下的响应性能。
  - **Redis 分布式支持**: 接入 Redis 缓存系统存储 CSRF Token，完美支持多 Worker 进程部署模式。
  - **自动内存降级**: 在 Redis 未配置时，自动降级为单机内存存储，确保系统兼容性。
- **审计日志脱敏优化**: 
  - 在 `backend/core/middleware.py` 中新增 `mask_sensitive_data` 递归脱敏工具。
  - 自动识别并遮蔽日志中的 `password`、`token`、`secret` 等敏感字段。

### 🏗️ 架构优化 (Architecture Optimization)
- **数据库会话管理优化**: 
  - 重构 `backend/core/database.py` 中的 `get_db` 和 `get_db_session`。
  - 采用标准的 `async with` 上下文管理器模式，利用 SQLAlchemy 的自动清理特性，提升资源释放的健壮性。

### 🚀 性能与稳定性全方位优化 (Comprehensive Optimization)
- **高性能 JSON 解析**: 后端引入 `orjson` 并将其设置为 FastAPI 默认响应类，大幅提升高并发下的数据序列化效率。
- **无感登录刷新 (Silent Refresh)**: 
  - 前端 `api.js` 接入 Refresh Token 机制。
  - 当访问令牌过期导致 401 错误时，系统自动尝试静默刷新令牌并重试原请求，显著提升用户连续操作体验。
- **WebSocket 通信加固**: 
  - **指数退避重连**: 优化 `WebSocketClient` 重连策略，采用指数级延迟增长（最高 30s），降低服务器瞬时重连压力。
  - **心跳断线检测**: 引入心跳超时机制（10s 阈值），主动发现并修复“假在线”连接，确保实时状态同步的准确性。
- **网络错误边界 (Error Boundary)**: 
  - 前端 `Api.request` 增加对 `TypeError: Failed to fetch` 的识别与拦截。
  - 网络断开时显示友好的统一提示，避免抛出不可读错误，并增加 5 秒防抖防止重复弹窗。
- **数据库连接池监控**: 
  - 新增 `get_pool_status()` 函数，暴露连接池健康指标（活跃连接数、空闲连接、溢出、使用率）。
  - 指标已整合至 `/api/v1/monitor/system` 接口，便于管理员实时监控数据库瓶颈。
- **缓存预热机制 (Cache Warmup)**: 
  - 新增 `warm_cache()` 函数，在系统启动时可选预加载角色权限、系统设置、近期公告至 Redis。
  - 通过环境变量 `CACHE_WARM_ON_STARTUP=true` 启用，降低冷启动后的数据库压力。
- **WebSocket 断线 UI 指示**: 
  - 顶栏 `TopBarComponent` 监听 WebSocket 连接状态。
  - 断线时显示醒目的"离线"标识，提升用户对系统实时状态的感知。
- **前端路由懒加载 (Route Lazy Loading)**: 
  - 重构 `app.js` 的模块加载逻辑，JS 文件不再在应用初始化时同步加载。
  - 由 `Router.handleRoute()` 在导航时按需调用 `ResourceLoader.loadModuleByPath()` 加载所需资源。
  - 新增路由加载指示器：当资源加载超过 200ms 时显示加载动画，避免用户困惑。
  - **预期效果**：首屏加载时间（TTI）显著缩短，仅加载当前页面所需资源。

---

## [v2.5.13] - 2026-02-07 (测试框架架构修复)

### 🧪 测试框架重构 (Test Framework Refactoring)

#### 🔧 核心修复
- **测试数据库隔离问题修复**: 彻底解决了 SQLite 内存数据库在测试环境中的连接隔离问题。
  - 问题根源：SQLite 内存数据库 (`sqlite+aiosqlite:///:memory:`) 对每个新连接都会创建独立的数据库实例，导致测试中创建的表在 API 调用时不可见。
  - 修复方案：重构测试配置，使用全局引擎代替独立的测试引擎，确保所有数据库操作使用同一个内存数据库实例。

- **Pytest 配置结构优化**: 
  - 新增 `backend/conftest.py` 作为 pytest 入口点，负责设置测试环境变量。
  - 保留 `backend/tests/test_conftest.py` 作为所有 fixtures 的主要定义文件。
  - 这种结构确保 `tests/` 和 `modules/` 两个不同目录的测试都能共享同一套 fixtures。

- **模块测试导入路径修复**: 修复了 3 个模块测试文件中的导入路径拼写错误：
  - `modules/pdf/pdf_tests/test_pdf_service.py`
  - `modules/markdown/markdown_tests/test_markdown.py`
  - `modules/lm_cleaner/lm_cleaner_tests/test_lm_cleaner.py`
  - 修正：`tests.tests_conftest` → `tests.test_conftest`

#### ✅ 验证结果
- **完整测试通过**: 406 个测试全部通过（175 个系统测试 + 231 个模块测试）

---

## [v2.5.12] - 2026-02-06 (系统设置获取修复)

### 🐛 问题修复 (Bug Fixes)
- **Store 核心修复**: 修复了 `Store.getSystemSettings` 方法缺失导致的 `TypeError` 运行时错误。
  - 该问题曾导致个人中心修改密码、用户管理等模块无法正常获取系统配置（如密码最小长度限制）。
  - 在 `frontend/js/core/store.js` 中补全了 `getSystemSettings` 方法，确保系统设置的正确读取。
