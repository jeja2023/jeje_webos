...
[![Version](https://img.shields.io/badge/version-2.1.3-blue.svg)](https://github.com/jeja2023/jeje_webos)
...
### v2.1.3 (2026-01-01)
- 🎨 **图表系统重构**：
  - **图表工厂类**：新增统一的 `ChartFactory` 工具类，实现图表配置标准化，消除代码重复。
  - **代码优化**：重构图表分析模块，大幅简化图表生成逻辑，提升可维护性。
- 🚀 **ETL 引擎增强**：
  - **新算子支持**：新增文本处理（大小写转换、去空格、长度计算等）、数学运算、窗口函数等高级算子。
  - **缓存优化**：ETL 缓存自动清理机制，自动删除超过 24 小时的过期缓存文件。
- 💄 **前端体验优化**：
  - **导入模块优化**：大幅优化数据导入界面和交互体验。
  - **对比分析增强**：优化数据集对比功能的 UI 和数据处理逻辑。
  - **智能表格优化**：增强智能表格的数据展示和编辑功能。
  - **样式统一**：统一分析模块各子功能的样式风格，提升视觉一致性。

### v2.1.2 (2026-01-01)
- 🚀 **ETL 数据建模增强**：
  - **新算子加入**：新增「文本处理」、「数学运算」、「窗口函数」三大类算子，支持更复杂的数据清洗与分析。
  - **SQL 脚本优化**：SQL 算子图标更新为终端样式 (💻)，配置体验更佳。
- 💄 **UI/UX 优化**：
  - **算子库布局**：侧边栏加宽至 260px，算子列表改为 4 列布局，显示更完整清晰。
  - **节点样式**：节点高度增加，确保算子名称在各种分辨率下均能完整显示。
- 🔧 **后端执行引擎升级**：
  - **高性能运算**：新增算子底层基于 DuckDB 高效实现，支持千万级数据秒级处理。
...
### v2.1.0 (2025-12-25)
- 👁️ **数据透镜模块深度优化**：
  - **Spotlight 联动**：支持全局搜索视图 (`Ctrl + K`) 并直接唤起独立窗口
  - **性能飞跃**：实现 CSV/Excel 内存缓存机制，大幅提升大文件分页浏览流畅度
  - **独立窗口体系**：支持多开对比，主 Hub 与视图窗口路由完全解耦
- 🐛 **问题修复**：
  - 修复筛选面板逻辑（默认空条件、关闭清空、过滤无效字段）
  - 修复图表初始化失败、HTML 标签格式错误、独立窗口标题重复等问题
- 💄 **UI/UX 细节体验打磨**：
  - **专业导航**：新增面包屑层级显示
  - **对比度增强**：修复预览按钮可见性，引入呼吸灯引导动画
- 📖 **文档中心更新**：
  - 更新「帮助中心」详细指南及“独立窗口架构”设计规范
...
- 🎯 **BI 仪表盘**：独立模块，支持创建仪表盘、添加可视化组件、网格布局、数据持久化。
- 🐛 **主题修复**：修复深色主题 Dock 提示框文字颜色、系统深色模式按钮文字问题。
