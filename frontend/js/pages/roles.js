...
class RolesPage extends Component {
    ...
    openEditor(role = null) {
        ...
        if (isAdmin) {
            ...
            featuresHtml = `
                <div style="padding:12px;background:var(--bg-secondary);border-radius:4px;margin-bottom:12px;">
                    ...
                    <div style="color:var(--text-secondary);font-size:13px;">
                        管理员可以管理普通用户和访客，但无法管理系统管理员和其他管理员
                    </div>
                </div>
                <div style="color:var(--text-secondary);font-size:13px;padding:8px;background:var(--bg-tertiary);border-radius:4px;">
                    ...
                    <div style="display:flex;gap:12px;flex-wrap:wrap;">
                        ${list.map(f => `
                            <label style="display:flex;align-items:center;gap:6px;">
                                <input type="checkbox" name="perms" value="${f.id}" ${perms.includes(f.id) ? 'checked' : ''}>
                                <span>${f.label}</span>
                            </label>
                        `).join('')}
                    </div>
        ...
        Modal.show({
            title: isEdit ? `编辑用户组 - ${role.name}` : '新建用户组',
            content: `
                <form id="roleForm">
                    <div class="form-group">
                        <label class="form-label">名称</label>
                        <input type="text" class="form-input" name="name" value="${Utils.escapeHtml(role?.name || '')}" placeholder="用户组名称" required ${isSystemRole ? 'readonly' : ''}>
