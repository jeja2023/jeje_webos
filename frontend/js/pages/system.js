...
class SystemSettingsPage extends Component {
    ...
    async handleSubmit(e) {
        ...
        const payload = {
            ...
            password_min_length: parseInt(form.password_min_length.value),
            jwt_expire_minutes: parseInt(form.jwt_expire_minutes.value),
            login_fail_lock: parseInt(form.login_fail_lock.value),
            jwt_rotate_enabled: form.jwt_rotate_enabled.checked
        };
                ...
                <div class="card">
                    <form id="systemSettingsForm" class="card-body">
                        ...
                        <div style="display:flex;gap:12px;">
                            <button type="submit" class="btn btn-primary" ${saving ? 'disabled' : ''}>${saving ? '保存中...' : '保存设置'}</button>
    ...
    bindEvents() {
        const form = this.$('#systemSettingsForm');
        if (form && !form._bindedSettings) {
            form._bindedSettings = true;
            form.addEventListener('submit', (e) => this.handleSubmit(e));
        }
        const reloadBtn = this.$('#reloadSettings');
        if (reloadBtn && !reloadBtn._bindedSettings) {
            reloadBtn._bindedSettings = true;
            reloadBtn.addEventListener('click', () => this.loadData());
        }
    }
}
