...
const DataLensHubMixin = {
    ...
    _toggleStartMenuShortcut(view, isSaved) {
        try {
            ...
            try {
                ...
                pinnedApps = saved ? JSON.parse(saved) : [];
            } catch (e) {
                pinnedApps = [];
            }
            ...
            if (isSaved) {
                // 从固定列表中移除
                pinnedApps = pinnedApps.filter(app =>
                    !(app.type === 'datalens' && app.view_id === view.id)
                );
                ...
                // 避免重复添加
                if (!pinnedApps.some(app => app.type === 'datalens' && app.view_id === view.id)) {
            ...
            // 更新 Store 以触发开始菜单等组件实时刷新
            const user = Store.get('user');
            if (user) {
                const shortcuts = pinnedApps.filter(app => app.type === 'datalens');
                const newSettings = {
                    ...(user.settings || {}),
    ...
    _renderViewCards(views) {
        if (views.length === 0) {
            return `
                <div class="lens-empty">
                    ...
                    <div class="lens-empty-hint" style="font-size: 14px; color: var(--text-muted); margin-top: -10px;">
        ...
        // 检查视图是否在开始菜单中
        const isPinned = (viewId) => pinnedApps.some(app => app.type === 'datalens' && app.view_id === viewId);
