...
const AnalysisSmartTableMixin = {
    ...
    renderSmartTable() {
        ...
        return `
            <div class="p-20">
                ...
                <div class="smart-table-grid" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(220px, 1fr)); gap: 20px;" id="smart-table-list">
                    ${this.state.smartTables ? this.state.smartTables.map(t => `
                        <div class="smart-table-card" style="border-radius: 10px; overflow: hidden; box-shadow: 0 4px 15px rgba(0,0,0,0.3); border: 1px solid var(--color-border);">
                            <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding: 25px; text-align: center;">
                                <div style="font-size: 32px;">📋</div>
                            </div>
                            <div style="padding: 15px; background: var(--color-card);">
    ...
    renderSmartTableDetail() {
        ...
        return `
            <div class="flex-column h-100">
                ...
                <div class="data-table-container" style="flex: 1; overflow: auto; padding: 20px;">
                    <table class="premium-table" style="border: 1px solid var(--color-border); border-collapse: collapse;">
                        <thead>
                            <tr>
                                ${table.fields.map(f => `<th style="border: 1px solid var(--color-border); padding: 12px;">${f.label || f.name}${f.type === 'calculated' ? ' ⚡' : ''}</th>`).join('')}
                                <th style="border: 1px solid var(--color-border); padding: 12px;" width="100">操作</th>
                            </tr>
                        </thead>
                        <tbody>
                            ${data.map(row => `
                                <tr>
                                    ${table.fields.map(f => `<td style="border: 1px solid var(--color-border); padding: 10px;">${formatCellValue(f, row)}</td>`).join('')}
                            ...
                            `).join('')}
                            ${data.length === 0 ? `<tr><td colspan="${table.fields.length + 1}" class="text-center p-20" style="border: 1px solid var(--color-border);">暂无数据</td></tr>` : ''}
                        </tbody>
                    </table>
                </div>
            </div>
    ...
    showSmartTableModal(table = null) {
        ...
        const renderFields = () => {
            return fields.map((f, i) => {
                ...
                return `
                <div class="field-setup-item p-12 mb-10 border-radius-sm bg-light relative" data-index="${i}" 
                     style="border: 1px solid var(--color-border); width: ${isCalc && !f._collapsed ? '100%' : 'calc(50% - 5px)'}; flex-shrink: 0;">
                    ...
                    ${isCalc ? `
                        <div class="calc-config mt-10 p-12 bg-white border-radius-sm" style="border: 1px dashed var(--color-primary); display: ${f._collapsed ? 'none' : 'block'};">
    ...
    showSmartRowModal(rowData = null) {
        ...
        Modal.show({
            ...
            width: '900px',
            content: `
                <div class="smart-row-form" style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 15px;">
                    ${table.fields.map(f => {
                const isCalc = f.type === 'calculated';
                return `
