...
class HeaderComponent extends Component {
    constructor(container) {
        ...
        // 监听未读通知数变化
        Store.subscribe('unreadNotifications', (count) => {
    ...
    bindEvents() {
        // 侧边栏切换
        const toggleBtn = this.$('#toggleSidebar');
        ...
        if (userMenuToggle && userDropdownMenu && !userMenuToggle._bindedHeader) {
            ...
            userMenuToggle.addEventListener('click', (e) => {
                ...
                if (isShow) {
                    ...
                    // 设置位置并显示
                    this.portalMenu.style.top = (rect.bottom + 8) + 'px';
    ...
    bindPortalMenuEvents() {
        ...
        // 使用事件委托，避免重复绑定
        if (!this.portalMenu._eventsBinded) {
            this.portalMenu._eventsBinded = true;
            
            // 绑定下拉菜单项点击事件
            this.portalMenu.addEventListener('click', async (e) => {
                ...
                if (item) {
                    ...
                    switch (action) {
                        case 'profile':
                            ...
                            break;
                        case 'logout':
                            Modal.confirm('退出登录', '确定要退出登录吗？', async () => {
                                try {
